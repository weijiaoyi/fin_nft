...
class TransferLogic
{
    ...
    public function transfer($userInfo, $target_phone, $currency_id, $account, $pay_password)
    {
        ...
        $config = $this->configTransferData->where(['currency_id' => $currency_id, 'is_show' => 1, 'is_del' => 0])->find();
        if (empty($config)) return Response::fail('未开启转赠功能');
        if ($account < $config['min']) return Response::fail('最小转赠数量' . $config['min']);
        if ($account > $config['max']) return Response::fail('最大转赠数量' . $config['max']);
        if ($account % $config['cardinal_number']) return Response::fail('转赠基数为' . $config['cardinal_number']);
        $targetInfo = (new Users())->where(['phone' => $target_phone, 'is_del' => 0])->find();
        if (empty($targetInfo)) return Response::fail('接收人手机号错误');
        $uid = $userInfo['id'];
        $accountLogic = new AccountLogic();
        Db::startTrans();
        $result = $accountLogic->subAccount($uid, $currency_id, $account, '转赠', '向会员' . $targetInfo['phone'] . '转出');
        if (!$result) {
            Db::rollback();
        ...
        $result = $accountLogic->addAccount($target_uid, $currency_id, $reality_account, '转赠', '会员' . $userInfo['phone'] . '转入');
        if (!$result) {
            Db::rollback();
