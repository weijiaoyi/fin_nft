 ...
 * @icon fa fa-circle-o
 */
class MangheGoods extends Backend
{
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $list = $this->model
                ->where(['goods.is_del' => 0,'goods.is_manghe_goods'=>1])
                ->with(['goodsrank'])
                ->where($where)
                ->order($sort, $order)
                ->paginate($limit);
            ...
            foreach ($list as $row) {
                $row->visible(['id', 'name', 'title', 'image', 'images', 'price', 'type', 'content', 'order', 'start_time', 'end_time', 'stock', 'sales', 'surplus', 'company_name', 'company_image', 'creator', 'owner', 'casting_name', 'casting_time', 'blockchain', 'contract_address', 'is_show', 'coupon_id', 'part', 'goods_category_id', 'is_manghe', 'is_can_buy', 'is_chip']);
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                $result = false;
//                {"attributes":[],"image":"https:\/\/nft2.mianqianba.com\/uploads\/20220507\/be00951e38dfe66fa234231d1e781b4a.jpg","name":""}
                Db::startTrans();
                try {
                    //是否采用模型验证
